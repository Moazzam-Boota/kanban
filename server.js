...
function getDocs(res, type, cronjob = false) {
  ...
  pouchDBConnection.allDocs(
    ...
    function (err, response) {
      var filterRows = [];
      var excelRows = [];
      let filterRowsMaxDate = response.rows
          .filter((i) => i.doc.type === "excel")
          .map((d) => moment(d.doc.createdAt)),
      ...
      response.rows.map((i) => {
        ...
        if (
          i.doc.type === "excel" &&
          singleMaxDate.isSame(moment(i.doc.createdAt))
        ) {
...
schedule.scheduleJob(" * * * * * ", function () {
  ...
  remoteDB
    ...
    .then(function (res) {
      ...
      var timeString = "";
      timeString = lodash.get(
        reversedsortedShifs,
        "[0].values.downloadTime",
        ""
      );
      ...
      if (timeString === moment().format("HH:mm")) {
        ...
        const downloadingTime = new Date(
          year,
          month,
          day,
          hours,
  ...
  workbook.xlsx.load(req.files.file.data).then(function () {
    ...
    worksheet.eachRow(function (row, rowNumber) {
      ...
      if (row.getCell("EF").value === "PERS012") {
        ...
        var data = {
          _id:
            new Date().toISOString().slice(0, 10) + Math.random().toString(36),
          type: "excel",
